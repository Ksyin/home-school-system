...
async function bootTutorPortfolios() {
  console.log('🎯 Booting tutor portfolios (parent-equivalent mode)...');
  ...
  try {
    ...
    // Use SAME loading method as parent - load all portfolios created by tutor
    const portfolios = await loadPortfoliosForTutor(user.uid);
    console.log(`📁 Rendering ${portfolios.length} portfolios for tutor (parent mode)`);
    
    pageContent.innerHTML = renderPortfolioGridForTutor(portfolios, profile);
    
    // Attach event handlers for tutor-specific actions
    attachTutorPortfolioEventHandlers();
    ...
    document.getElementById('page-content').innerHTML = `
      <div class="card panel" style="text-align:center;padding:40px;">
        ...
        <button class="btn" onclick="showCreatePortfolioModal('tutor', '${auth.currentUser?.uid}', '${(auth.currentUser?.displayName || 'Tutor').replace(/'/g, "\\'")}')">
...
function attachTutorPortfolioEventHandlers() {
  // Any additional event handlers for tutor portfolio page
  console.log('Tutor portfolio event handlers attached');
}
// NEW: Render portfolio grid for tutor - same as parent's grid
function renderPortfolioGridForTutor(portfolios, profile) {
  const canCreate = true; // Tutors can always create
  ...
  const renderPortfolioCard = (p) => {
    const progressBar = p.totalStudents > 0 
      ? `<div class="portfolio-progress">
           <div class="progress-bar-container">
             <div class="progress-bar-fill" style="width: ${(p.completedCount / p.totalStudents) * 100}%;"></div>
           </div>
           <span class="progress-text">${p.completedCount}/${p.totalStudents} completed</span>
         </div>`
      : '';
    
    return `
      <div class="portfolio-grid-card" data-portfolio-id="${p.id}" onclick="openPortfolioForTutor('${p.id}')">
        ...
        <div class="portfolio-card-content">
          ...
          <div class="portfolio-card-footer">
            <span class="portfolio-creator">👥 ${p.totalStudents || 0} students</span>
            ${p.dueDate ? `<span class="portfolio-due">📅 Due ${fmtDate(p.dueDate)}</span>` : ''}
          </div>
          
          <div class="portfolio-card-actions" style="margin-top:12px;display:flex;gap:8px;justify-content:flex-end;">
            <button class="btn small ghost" onclick="event.stopPropagation(); editPortfolio('${p.id}')">✏️ Edit</button>
            <button class="btn small ghost" onclick="event.stopPropagation(); viewPortfolioSubmissions('${p.id}')">📊 Submissions</button>
            <button class="btn small danger" onclick="event.stopPropagation(); deletePortfolio('${p.id}')">🗑️ Delete</button>
          </div>
        </div>
      </div>
    `;
  ...
  return `
    <style>
      /* Portfolio Grid Styles - Same as parent */
    ...
    <div class="portfolio-page-container">
      <div class="portfolio-header">
        ...
        <button class="btn" onclick="showCreatePortfolioModal('tutor', '${auth.currentUser?.uid}', '${(profile?.name || profile?.full_name || 'Tutor').replace(/'/g, "\\'")}')">
          ➕ Create Portfolio
        </button>
      </div>
      ...
      ` : `
        <div class="empty-state">
          ...
          <p>Create your first portfolio to assign to students!</p>
          <button class="btn" style="margin-top:20px;" onclick="showCreatePortfolioModal('tutor', '${auth.currentUser?.uid}', '${(profile?.name || profile?.full_name || 'Tutor').replace(/'/g, "\\'")}')">
            ➕ Create Portfolio
          </button>
        </div>
      `}
...
// NEW: Open portfolio for tutor view (same as parent but with tutor controls)
window.openPortfolioForTutor = async function(portfolioId) {
  const bundle = await requireAuth();
  if (!bundle) return;
  ...
  try {
    ...
    // Load all student entries for this portfolio
    ...
    allEntries.forEach(entry => {
      if (!entriesByStudent.has(entry.studentId)) {
        entriesByStudent.set(entry.studentId, []);
      }
      entriesByStudent.get(entry.studentId).push(entry);
    });
    
    // Load student details for assigned students
    const studentIds = portfolio.studentIds || [];
    const students = [];
    for (const studentId of studentIds) {
      ...
      if (studentDoc.exists()) {
        ...
        students.push({
          id: studentId,
          name: studentData.full_name || studentData.name || studentData.email || 'Student',
          entries: studentEntries,
          completedCount: studentEntries.length,
          totalSections: sections.length,
          percentage: sections.length > 0 ? Math.round((studentEntries.length / sections.length) * 100) : 0
        });
      }
    }
    
    // Store current view state
    window.currentPortfolioView = { portfolio, sections, students, allEntries };
    
    // Render the tutor portfolio view
    const content = document.getElementById('page-content');
    content.innerHTML = renderTutorPortfolioView(portfolio, sections, students);
...
// NEW: Render tutor portfolio view (shows all students' submissions)
function renderTutorPortfolioView(portfolio, sections, students) {
  const sectionsHtml = sections.map((section, idx) => `
    <div class="section-header-mini">
      <strong>${idx + 1}. ${escapeHtml(section.title)}</strong>
      <span class="section-type-badge">${getSectionTypeIcon(section.type)} ${getSectionTypeLabel(section.type)}</span>
    </div>
  `).join('');
  
  const studentsHtml = students.map(student => {
    // Create a map of sectionId -> entry for this student
    const entryMap = new Map();
    student.entries.forEach(e => entryMap.set(e.sectionId, e));
    
    const studentSectionsHtml = sections.map(section => {
      const entry = entryMap.get(section.id);
      const hasContent = !!entry;
      
      return `
        <div class="student-section-cell ${hasContent ? 'completed' : 'pending'}">
          ${hasContent ? `
            <div class="cell-preview" onclick="viewStudentSectionResponse('${portfolio.id}', '${student.id}', '${section.id}')">
              ${entry.content ? `<span>📝 ${escapeHtml(entry.content.substring(0, 50))}${entry.content.length > 50 ? '...' : ''}</span>` : ''}
              ${entry.fileUrl ? `<span>📎 File attached</span>` : ''}
              ${!entry.content && !entry.fileUrl ? '<span>✓ Completed</span>' : ''}
            </div>
        ...
        <div class="student-info-cell">
          ...
          <div class="student-progress-mini">
            <div class="progress-bar-mini">
              <div class="progress-fill-mini" style="width: ${student.percentage}%;"></div>
            </div>
            <span>${student.completedCount}/${student.totalSections}</span>
          </div>
        </div>
        ${studentSectionsHtml}
        <div class="student-actions-cell">
          <button class="btn small ghost" onclick="messageStudentAboutPortfolio('${student.id}', '${escapeHtml(student.name)}', '${portfolio.id}')">💬 Message</button>
          <button class="btn small ghost" onclick="downloadStudentPortfolio('${portfolio.id}', '${student.id}')">📥 Download</button>
        </div>
      </div>
    `;
  }).join('');
  ...
  return `
    <style>
      ...
      .table-header {
        display: grid;
        grid-template-columns: 200px repeat(${sections.length}, minmax(120px, 1fr)) 140px;
        background: #f8f9fa;
        border-bottom: 2px solid #e0e0e0;
        font-weight: 600;
        padding: 16px;
      ...
      .student-section-cell {
        padding: 8px;
        text-align: center;
      }
      
      .student-section-cell.completed {
        background: #e8f5e9;
        border-radius: 8px;
      }
      
      .student-section-cell.pending {
        color: #999;
      }
    ...
    <div class="tutor-portfolio-view">
      <div class="back-button" onclick="closePortfolioView()">
        ← Back to Portfolios
      </div>
      
      <div class="portfolio-header-large">
        <div class="portfolio-title-row">
          ...
          <div>
            ...
            <h2 style="margin: 8px 0 4px 0;">${escapeHtml(portfolio.title)}</h2>
            <p style="color:#666;margin:0;">${escapeHtml(portfolio.description || '')}</p>
          </div>
        </div>
        
        <div class="portfolio-stats">
          <span class="stat-badge">👥 ${students.length} students</span>
          <span class="stat-badge">📋 ${sections.length} sections</span>
          <span class="stat-badge">✅ ${students.filter(s => s.percentage === 100).length} completed</span>
          ${portfolio.dueDate ? `<span class="stat-badge">📅 Due ${fmtDate(portfolio.dueDate)}</span>` : ''}
        </div>
      </div>
      ...
      <div class="students-table">
        <div class="table-header">
          <div>Student</div>
          ${sections.map(s => `<div class="section-header-mini" title="${escapeHtml(s.title)}">${escapeHtml(s.title.substring(0, 20))}${s.title.length > 20 ? '...' : ''}</div>`).join('')}
          <div>Actions</div>
        </div>
        ${studentsHtml}
      </div>
      ...
      ${students.length === 0 ? `
        <div class="empty-state" style="text-align:center;padding:40px;">
          <p>No students assigned to this portfolio yet.</p>
          <button class="btn" onclick="editPortfolio('${portfolio.id}')">✏️ Edit Portfolio to Add Students</button>
        </div>
      ` : ''}
    </div>
  `;
}
// NEW: View individual student's section response
window.viewStudentSectionResponse = async function(portfolioId, studentId, sectionId) {
  try {
    ...
    const studentName = studentDoc.exists() 
      ? (studentDoc.data().full_name || studentDoc.data().name || 'Student')
      : 'Student';
    
    const sectionDoc = await getDoc(doc(db, 'portfolio_sections', sectionId));
    const sectionTitle = sectionDoc.exists() ? sectionDoc.data().title : 'Section';
    ...
    modal.innerHTML = `
      <div class="modal-overlay" onclick="this.parentElement.remove()">
        <div class="modal-box" style="max-width:600px;" onclick="event.stopPropagation()">
          <div class="modal-header">
            <h3>${escapeHtml(studentName)} - ${escapeHtml(sectionTitle)}</h3>
            <button class="btn danger" onclick="this.closest('.modal-overlay').parentElement.remove()">✕</button>
          </div>
          <div class="modal-body">
            ${entry?.content ? `<p style="white-space:pre-wrap;">${escapeHtml(entry.content)}</p>` : ''}
            ${entry?.fileUrl ? renderFilePreview(entry.fileUrl, entry.fileName) : ''}
            ${!entry?.content && !entry?.fileUrl ? '<p class="empty">No response yet</p>' : ''}
            <div style="margin-top:16px;">
              <label>Add Feedback:</label>
              <textarea id="feedbackText" rows="4" style="width:100%;padding:12px;margin-top:8px;" placeholder="Provide feedback to the student...">${entry?.feedback || ''}</textarea>
              <button class="btn" style="margin-top:12px;" onclick="saveStudentFeedback('${portfolioId}', '${studentId}', '${sectionId}', this)">Save Feedback</button>
            </div>
          </div>
        </div>
      </div>
...
// NEW: Save feedback on student's section
window.saveStudentFeedback = async function(portfolioId, studentId, sectionId, btnElement) {
  ...
  try {
    ...
    if (!entrySnap.empty) {
      await updateDoc(doc(db, 'portfolio_entries', entrySnap.docs[0].id), {
        feedback: feedbackText,
        feedbackGivenAt: serverTimestamp(),
        feedbackGivenBy: auth.currentUser.uid
      });
      
      // Create notification for student
      await addDoc(collection(db, 'notifications'), {
        studentId: studentId,
        title: 'Feedback on Your Portfolio',
        message: `Your tutor left feedback on ${sectionId}`,
...
// NEW: Message student about portfolio
window.messageStudentAboutPortfolio = async function(studentId, studentName, portfolioId) {
  const message = prompt(`Message to ${studentName}:\n\nEnter your message about the portfolio:`);
  if (!message) return;
  ...
  try {
    ...
    await addDoc(collection(db, 'notifications'), {
      studentId: studentId,
      title: 'New Message About Your Portfolio',
...
// NEW: Download student portfolio as JSON/HTML
window.downloadStudentPortfolio = async function(portfolioId, studentId) {
  try {
    const portfolioDoc = await getDoc(doc(db, 'portfolios', portfolioId));
    ...
    const html = `
      <!DOCTYPE html>
      <html>
      <head><meta charset="utf-8"><title>${portfolio.title} - ${studentName}</title>
      <style>
        ...
        ${sections.map(section => {
          ...
          return `
            <div class="section">
              ...
              ${section.description ? `<p><em>${escapeHtml(section.description)}</em></p>` : ''}
              <div class="response">
                <strong>Student Response:</strong>
                ${entry?.content ? `<p>${escapeHtml(entry.content)}</p>` : '<p>No response</p>'}
                ${entry?.fileUrl ? `<div class="file">📎 File: <a href="${entry.fileUrl}">${escapeHtml(entry.fileName || 'Download')}</a></div>` : ''}
                ${entry?.feedback ? `<div class="feedback"><strong>Tutor Feedback:</strong> <p>${escapeHtml(entry.feedback)}</p></div>` : ''}
              </div>
            </div>
          `;
        }).join('')}
    ...
    a.href = url;
    a.download = `${portfolio.title}_${studentName}.html`;
    a.click();
...
window.deletePortfolio = async function(portfolioId) {
  if (!confirm('Delete this portfolio and all student submissions? This cannot be undone.')) return;
  ...
  try {
    ...
    for (const docSnap of sectionsSnap.docs) {
      await deleteDoc(doc(db, 'portfolio_sections', docSnap.id));
    }
    ...
    for (const docSnap of entriesSnap.docs) {
      await deleteDoc(doc(db, 'portfolio_entries', docSnap.id));
    }
...
window.viewPortfolioSubmissions = async function(portfolioId) {
  try {
    const entriesSnap = await getDocs(
      query(collection(db, 'portfolio_entries'), where('portfolioId', '==', portfolioId))
    );
    ...
    entries.forEach(e => {
      if (!byStudent.has(e.studentId)) {
        byStudent.set(e.studentId, { name: e.studentName, count: 0, entries: [] });
      }
      byStudent.get(e.studentId).count++;
      byStudent.get(e.studentId).entries.push(e);
    });
    
    const summary = Array.from(byStudent.values())
      .map(s => `• ${s.name}: ${s.count} sections completed`)
      .join('\n');
...
async function loadPortfoliosForTutor(tutorId) {
  try {
    // Get all portfolios and filter by createdBy
    const snap = await getDocs(collection(db, 'portfolios'));
    const portfolios = snap.docs
      .map(d => ({ id: d.id, ...d.data() }))
      .filter(p => p.createdBy === tutorId);
    
    // Load completion stats for each portfolio
    for (const portfolio of portfolios) {
      // Count total students assigned
      const studentIds = portfolio.studentIds || [];
      portfolio.totalStudents = studentIds.length;
      ...
      for (const studentId of studentIds) {
        ...
        const sectionsSnap = await getDocs(
          query(collection(db, 'portfolio_sections'), where('portfolioId', '==', portfolio.id))
        );
        if (entriesSnap.size >= sectionsSnap.size && sectionsSnap.size > 0) {
          completedCount++;
        }
      }
      portfolio.completedCount = completedCount;
    }
...
const routeMap = {
  ...
  tutor: {
    ...
    'portfolio': bootTutorPortfolios,      // FIXED: Points to new function
