...
const navMap = {
  ...
  student: [
    ['Dashboard', '/student/dashboard.html', '🏠'],
        ['Classrooms', '/student/classrooms.html', '🏫'],  // ADD THIS LINE
...
const routeMap = {
  student: {
    ...
    'classrooms': bootStudentClassroomsPage,  // ADD THIS LINE
...
// Add after renderStudentMessagesPage

function renderStudentClassroomsPage(classrooms, selectedClassroomId, selectedClassroomData, assignments, resources, submissions) {
    ...
    return `
      <div class="card panel">
        ...
        <div class="empty-state" style="text-align:center;padding:60px;">
          <div style="font-size:48px;margin-bottom:16px;">🏫</div>
          <p>You haven't been assigned to any classroom yet.</p>
          <p style="color:#666;font-size:14px;">When your tutor adds you to a classroom, it will appear here.</p>
        </div>
      </div>
    `;
  }
  
  // Build classroom tabs
  const tabsHtml = classrooms.map(c => `
    <button class="classroom-tab ${selectedClassroomId === c.id ? 'active' : ''}" data-id="${c.id}">
      <span class="tab-icon">🏫</span>
      <span>${escapeHtml(c.name)}</span>
      <small>${escapeHtml(c.subject || 'General')}</small>
    </button>
  `).join('');
  
  // If no classroom selected, use first one
  const displayClassroom = selectedClassroomData || classrooms[0];
  const displayAssignments = assignments || [];
  const displayResources = resources || [];
  
  // Calculate stats for this classroom
  const submittedIds = new Set(submissions?.map(s => s.assignmentId) || []);
  const completedCount = displayAssignments.filter(a => submittedIds.has(a.id)).length;
  const pendingCount = displayAssignments.length - completedCount;
  
  // Assignments list
  const assignmentsHtml = displayAssignments.map(a => {
    const submitted = submittedIds.has(a.id);
    const isPastDue = a.dueDate && new Date(a.dueDate) < new Date();
    
    return `
      <div class="classroom-assignment-item" style="
        background: white;
        border-radius: 10px;
        padding: 16px;
        margin-bottom: 12px;
      ...
      ">
        <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:10px;">
          <div>
            ...
            <div style="display:flex;gap:16px;flex-wrap:wrap;font-size:13px;color:#666;">
              <span>📅 Due: ${fmtDate(a.dueDate) || 'No due date'}</span>
        ...
        ${a.description ? `<p style="margin-top:12px;font-size:14px;color:#555;">${escapeHtml(a.description.substring(0, 150))}${a.description.length > 150 ? '...' : ''}</p>` : ''}
      </div>
    `;
  }).join('');
  
  // Resources list
  const resourcesHtml = displayResources.map(r => `
    <div class="classroom-resource-item" style="
      background: #f8f9fa;
      border-radius: 10px;
      padding: 12px;
      margin-bottom: 8px;
    ...
    ">
      <div>
        <strong>${escapeHtml(r.title)}</strong>
        ${r.note ? `<p style="margin:4px 0 0;font-size:13px;color:#666;">${escapeHtml(r.note)}</p>` : ''}
      </div>
      ${r.fileUrl ? `<a href="${r.fileUrl}" target="_blank" class="btn small ghost">📎 View</a>` : ''}
    </div>
  `).join('');
  ...
  return `
    ...
    <div class="classroom-header">
      <h2 style="margin:0 0 8px 0;">${escapeHtml(displayClassroom.name)}</h2>
      <p style="margin:0;opacity:0.9;">${escapeHtml(displayClassroom.subject || 'General Classroom')}</p>
      <p style="margin-top:8px;font-size:14px;">${escapeHtml(displayClassroom.description || 'No description')}</p>
    </div>
    
    <div class="classroom-stats">
      <div class="classroom-stat-card">
        <div class="classroom-stat-number">${displayAssignments.length}</div>
        <p style="margin:0;color:#666;">Assignments</p>
      </div>
      <div class="classroom-stat-card">
        <div class="classroom-stat-number">${completedCount}</div>
        <p style="margin:0;color:#666;">Completed</p>
      </div>
      <div class="classroom-stat-card">
        <div class="classroom-stat-number">${pendingCount}</div>
        <p style="margin:0;color:#666;">Pending</p>
      </div>
    </div>
    
    <div class="card panel" style="margin-bottom:24px;">
      <h3 class="section-title">📝 Classroom Assignments</h3>
      ${assignmentsHtml || '<p class="empty">No assignments for this classroom yet.</p>'}
      ${displayAssignments.length > 5 ? '<div style="margin-top:12px;text-align:center;"><a href="/student/assignments.html" class="btn ghost">View All Assignments →</a></div>' : ''}
    </div>
    
    <div class="card panel">
      <h3 class="section-title">📚 Classroom Resources</h3>
      ${resourcesHtml || '<p class="empty">No resources shared for this classroom yet.</p>'}
    </div>
  `;
}

// Add this function after loadStudentAssignments

async function loadStudentClassrooms(studentUid) {
  console.log('🏫 Loading classrooms for student:', studentUid);
  
  // Get student's classroom assignments
  const studentDoc = await getDoc(doc(db, 'students', studentUid));
  const studentData = studentDoc.data();
  const studentClassroomId = studentData?.classroomId || '';
  
  // Get all classrooms
  const classroomsSnap = await getDocs(collection(db, 'classrooms'));
  const classrooms = [];
  
  for (const docSnap of classroomsSnap.docs) {
    const classroom = { id: docSnap.id, ...docSnap.data() };
    let isVisible = false;
    
    // Student can see their assigned classroom
    if (classroom.id === studentClassroomId) isVisible = true;
    
    // Student can see if they're in the studentIds array
    if (Array.isArray(classroom.studentIds) && classroom.studentIds.includes(studentUid)) isVisible = true;
    
    if (isVisible) classrooms.push(classroom);
  }
  
  return classrooms;
}

// Load classroom-specific assignments
...
// Load classroom-specific resources
async function loadClassroomResources(classroomId) {
  const snap = await getDocs(
    query(collection(db, 'resources'), where('classroomId', '==', classroomId))
  );
  return snap.docs.map(d => ({ id: d.id, ...d.data() }));
}

// Add after bootStudentSettingsPage

async function bootStudentClassroomsPage() {
  const bundle = await requireAuth();
  if (!bundle || bundle.profile?.role !== 'student') return;
  ...
  // Get URL params for selected classroom
  const params = new URLSearchParams(window.location.search);
  const selectedClassroomId = params.get('classroomId');
  
  // Load classrooms for this student
  const classrooms = await loadStudentClassrooms(user.uid);
  
  if (!classrooms.length) {
    document.getElementById('page-content').innerHTML = renderStudentClassroomsPage([], null, null, [], [], []);
    return;
  }
  
  // Determine which classroom to show
  let targetClassroom = classrooms[0];
  let targetAssignments = [];
  let targetResources = [];
  
  if (selectedClassroomId) {
    const found = classrooms.find(c => c.id === selectedClassroomId);
    if (found) targetClassroom = found;
  }
  
  // Load data for the selected classroom
  const [assignments, resources, submissions] = await Promise.all([
    loadClassroomAssignments(targetClassroom.id),
    loadClassroomResources(targetClassroom.id),
    loadStudentSubmissions(user.uid)
  ]);
  
  document.getElementById('page-content').innerHTML = renderStudentClassroomsPage(
    classrooms, 
    targetClassroom.id, 
    targetClassroom, 
    assignments, 
  ...
  document.querySelectorAll('.classroom-tab').forEach(tab => {
    tab.addEventListener('click', async () => {
      const classroomId = tab.dataset.id;
      // Navigate to same page with classroom parameter
      window.location.href = `/student/classrooms.html?classroomId=${classroomId}`;
    });
  });
}
